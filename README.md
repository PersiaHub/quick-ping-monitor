# 🚀 Persia Code v1.2 Stable



<img width="1401" height="880" alt="Screenshot (2325)" src="https://github.com/user-attachments/assets/ebdfb421-8e48-4b79-be0d-cb63f391f08a" />

<img width="1404" height="891" alt="Screenshot (2328)" src="https://github.com/user-attachments/assets/1387e594-3c96-438e-96d9-91974a3db0ee" />

<img width="1405" height="892" alt="Screenshot (2329)" src="https://github.com/user-attachments/assets/dd41b7d2-25e7-482d-a3f3-fc2a5e879169" />





















An enterprise-grade, minimalist network diagnostic layer designed to track host pings, HTTP web availability, and TCP port status. Built with a beautiful glassmorphic UI using Tailwind CSS and backed by a robust Node.js engine.

یک لایه تشخیصی و ابزار مانیتورینگ شبکه برای بررسی پینگ هاست‌ها، وضعیت در دسترس بودن وب‌سایت‌ها (HTTP) و باز یا بسته بودن پورت‌های TCP. طراحی شده با ظاهر مدرن گلس‌مورفیسم توسط Tailwind CSS و هسته قدرتمند Node.js.

---

## 🌍 Language / زبان

- [English](#english-documentation)
- [فارسی](#راهنمای-فارسی)

---

## English Documentation

### ✨ Features
- **ICMP Ping:** Real-time latency tracking for any Domain or IP.
- **TCP Port Audit:** Instantly check if specific ports (e.g., 443, 80, 22) are open or filtered.
- **HTTP/HTTPS Telemetry:** Verify website status codes and web server response times.
- **Dynamic Port Routing:** Fully prepared for dynamic deployment across global networks or domestic infrastructures.
- **Glassmorphism UI:** Clean, responsive, and dark-themed dashboard.

### 🗂️ Project Structure
```text
quick-ping-monitor/
├── LICENSE
├── README.md
├── frontend/
│   └── index.html
└── backend/
    ├── server.js
    ├── package.json
    └── package-lock.json

⚡ Installation & Local Setup
1. Clone the Repository
git clone [https://github.com/PersiaHub/quick-ping-monitor.git](https://github.com/PersiaHub/quick-ping-monitor.git)

cd quick-ping-monitor

2. Initialize Backend Dependencies
Navigate to the backend folder and install the required network utilities:

cd backend
npm install

3. Run the Backend Server

node server.js
The core engine will start listening on port 5000.

4. Launch Front-End
Simply navigate to the frontend directory and open index.html in any modern web browse

⚖️ License
Distributed under a Proprietary License. Permission is granted for personal usage and execution only. Copying, redistribution, modifying, or removing architecture credits (@PersiaHub) is strictly prohibited. See LICENSE for details.


راهنمای فارسی

⚡ راه اندازی و اجرا
 دانلود پروژه
git clone [https://github.com/PersiaHub/quick-ping-monitor.git](https://github.com/PersiaHub/quick-ping-monitor.git)
cd quick-ping-monitor


۲. نصب نیازمندی‌های بک‌اند

اول وارد پوشه بک اند شوید و با زدن دکمه شیفت و کلیک راست موس 
open powershell window here

 موارد زیر را نصب کنید :

cd backend
npm install


۳. روشن کردن سرور بک‌اند

node server.js

بک‌اند پروژه روی پورت 5000 آماده به کار می‌شود.


۴. وارد مرورگر شوید و localhost:5000 سرچ کنید



⚖️ قوانین انتشار و لایسنس
این پروژه تحت لایسنس اختصاصی مالکیت معنوی منتشر شده است. استفاده شخصی و اجرای آن کاملاً مجاز است؛ اما هرگونه کپی‌برداری، بازنشر، دستکاری کدها یا حذف نام طراح اثر (@PersiaHub) شرعاً و قانوناً ممنوع می‌باشد. اطلاعات بیشتر در فایل LICENSE درج شده است.

Core Architect: @PersiaHub 🚀






"# quick-ping-monitor" 
